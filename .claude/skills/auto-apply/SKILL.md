...
| Location autocomplete rejects typed value on submit | Ashby location is a geocoder typeahead — needs actual dropdown selection | Try label texts "Where are you currently located?", "Location", etc.; fall back to `input[placeholder="Start typing..."]` (unique to location). Type city name only char-by-char (strip `, CA` suffix), wait 2 s, click `[role="option"]:first` |
| Yes/No eligibility questions not answered | Ashby eligibility questions use `<button>` elements, not radio inputs or labels | Use `page.locator('button').filter(has_text=re.compile(r'^\s*Yes\s*$'))` and loop — never `.first`, never target `<label>` |
